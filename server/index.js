...
let emergencyStatus = {
  ...
  type: '',
  timestamp: null,
  response: 'none'  // 'none', 'accept', or 'deny'
};

// ESP32 sends emergency notification
app.post('/api/emergency', (req, res) => {
  try {
    ...
    res.json({ success: true, message: 'Emergency notification received' });
  } catch (error) {
    console.error('Emergency notification error:', error);
    res.status(500).json({ success: false, error: 'Failed to process emergency' });
  }
});

// ESP32 checks for staff response
app.get('/api/emergency/status', (req, res) => {
  res.json({
    ...
    response: emergencyStatus.response
  });
});

// Staff page gets emergency status
...
app.post('/api/emergency/respond', (req, res) => {
  try {
    ...
    emergencyStatus.response = response;
    
    console.log(`✅ Staff responded: ${response.toUpperCase()}`);
...
// Clear emergency (after resolved)
